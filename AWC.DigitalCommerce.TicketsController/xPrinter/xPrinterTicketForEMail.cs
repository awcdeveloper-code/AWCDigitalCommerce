...
{
    public class xPrinterTicketForEMail
    {
        private string workVar = string.Empty;
        private clsTicketsForDataGrid ticket = new clsTicketsForDataGrid();

        public xPrinterTicketForEMail()
        {

        }

        public xPrinterTicketForEMail(clsTicketsForDataGrid _ticket)
        {
            ticket = _ticket;
        }
        ...
        public void print()
        {
            ...
            try
            {
                ...
                int ticketHeight = 825 + (lstItems.Count * 18);
                ...
                using (Graphics graphics = Graphics.FromImage(bitmap))
                {
                    ...
                    {
                        Offset += 10;
                        workVar = " TOTAL VENTA : " + (totalPrice + ticket.ServiceFee).ToString("N0").PadLeft(7);
                        ...
                        workVar = "  POR COBRAR : " + (totalPrice + ticket.ServiceFee + totalCash).ToString("N0").PadLeft(7);
                        graphics.DrawString(workVar, new Font("Consolas", 10), new SolidBrush(Color.Black), startX, startY + Offset);
                        Offset += 30;
                    }
                    else
                    {
                        ...
                        // TICKET TOTAL
                        string tot = (totalPrice + ticket.ServiceFee).ToString("N0");
                        ...
                        {
                            int totalUSD = (int)Math.Ceiling((double)(totalPrice + ticket.ServiceFee) / (double)Settings.Default.USDollarExchangeRate);
