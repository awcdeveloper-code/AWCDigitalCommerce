...
{
    public class xPrinterMealsSummary
    {
        ...
        public xPrinterMealsSummary()
        {

        }

        public xPrinterMealsSummary(List<clsItemDetailForDatagrid> _mealList)
        {
            mealList = _mealList;
        }
        ...
        void pdoc_PrintPage(object sender, PrintPageEventArgs e)
        {
            ...
            graphics.DrawString(new string(' ', 3) + "CIERRE DE COCINA", new Font("Consolas Bold", 12), new SolidBrush(Color.Black), startX, startY + Offset);
            Offset += 25;
            ...
            {
                workVar = new string(' ', 13) + "SUBTOTAL: " + tot.PadLeft(7);
                graphics.DrawString(workVar, new Font("Consolas", 8), new SolidBrush(Color.Black), startX, startY + Offset);
                Offset += 18;

                reduce10percent = totalPrice * 0.1;

                string red10 = reduce10percent.ToString("N0");
                workVar = new string(' ', 18) + "10%: " + red10.PadLeft(7);
                graphics.DrawString(workVar, new Font("Consolas", 8), new SolidBrush(Color.Black), startX, startY + Offset);
                Offset += 18;
                ...
                totalPrice = totalPrice - Convert.ToInt32(reduce10percent);
                tot = totalPrice.ToString("N0");
            }
